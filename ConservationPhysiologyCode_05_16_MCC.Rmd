...
### Mass
```{r Mass qq plots, fig.align="left"}
...
## Step 4: Are metrics correlated?
   ...
   + All metrics were positively corellated, but some had stronger correllation than others
...
**Overall Conclusion**
  ...
  + Based on LOESS models, all size metric s had a non-linear relationship with season day for at least one location, rending linear modeling not appropriate.
  + For body condition, examination of the LOESS model revealed a relatively linear relationship across season day at all locations, and so we proceeded with linear modeling to assess the relative contribution of different factors.
...
#names(summary(lmBodyC))
vc<-as.matrix(summary(lmBodyC)$cov.unscaled)
...
St Mary's Park
$Y=$ `r round(lm.to.print$coefficients[1],6)` $+$ `r round( lm.to.print$coefficients[2],6)` $xSD$ 

BNA
$Y=$ `r round(lm.to.print$coefficients[1]+lm.to.print$coefficients[3],6)` $+$ `r round( lm.to.print$coefficients[2]+lm.to.print$coefficients[5],6)` $xSD$ 

Ropes Course
$Y=$ `r round(lm.to.print$coefficients[1]+lm.to.print$coefficients[4],6)` $+$ `r round( lm.to.print$coefficients[2]+lm.to.print$coefficients[6],6)` $xSD$
...
d <- 15  # using variable d instead of hard coding 15 in case you 
         # want to change the day to 20 or something else later
...
# Sanity check: the following two outputs should match:
sum(b.h*val) + c(-1,1)*qt(0.975, d.f)*sqrt(sum(val%*%t(val) * vc))
as.matrix(10*confint(lmBodyC))[2,]
